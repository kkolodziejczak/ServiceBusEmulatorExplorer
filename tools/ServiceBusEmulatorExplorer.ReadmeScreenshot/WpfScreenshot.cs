...
internal static class WpfScreenshot
{
    ...
    private const int CaptureHeight = 741;
    ...
    public static void SaveWindowContent(Window window, string outputPath)
    {
        ...
        if (width < MinimumWidth || height < MinimumHeight)
        {
            throw new InvalidOperationException(
                $"The rendered app surface is unexpectedly small: {width}x{height}. Expected at least {MinimumWidth}x{MinimumHeight}.");
        }
        ...
        Console.WriteLine($"Captured {width}x{height} README screenshot at {outputPath}");
    }
}
