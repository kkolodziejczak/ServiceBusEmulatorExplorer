...
8. In Dead letter, Replay uses checked messages, or the focused message when none are checked. Each gets a new message ID. Edit and Replay is available for one target and opens an editor with a generated ID; Cancel does not replay. Original DLQ messages remain unchanged.
...
Build passed with zero warnings and errors. The [rendered walkthrough](verification.md) passed 35 interaction and state checks. Screenshots were visually inspected after repairs, including the replay editor at 760×640 and 560×440.
