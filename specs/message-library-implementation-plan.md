...
8. Follow-up decision: support Send now / Schedule for later with **one scheduled instant for the whole run**, including CSV batches. No per-row CSV schedule. Other capabilities discovered in the [send-capability audit](message-library-send-capabilities-research.md) remain recommendations until approved.
...
Stop only stops further submissions and accounts for the in-flight call. It does not revoke acknowledged schedules. Cancellation of already-scheduled messages is a separate capability under consideration in the audit and is not implied by Stop. Session-only receipts/results still have the existing explicit export option; no durable scheduler/outbox is added. Template files never persist an absolute schedule or inherit the inspected message's old schedule. TTL editing remains outside the approved scope.
...
No app-managed Git, remote URL browsing, collection marketplace, cloud sharing, script evaluation, loops/grouped CSV, per-row/recurring schedules, cancellation of acknowledged schedules, TTL editing, binary templates, arbitrary SDK application-property types, load testing, parallel publishing, durable job queue, automatic retries/resume, template deletion UI or unrelated legacy cleanup in v1. Existing replay/delete behavior remains separate. The send-capability audit may motivate a subsequent approved scope amendment.
...
Follow-up capability audit: common-time scheduling was added after the user's explicit choice. Independent focused review identified an incomplete receipt export contract (SCH-001); the row/run fields, null behavior, Int64 string serialization and tests were added, and the reviewer closed the finding. The [capability research](message-library-send-capabilities-research.md) inventories all 14 public message properties and sender operations using primary sources. Remaining expansion suggestions are not approved requirements. This documentation pass did not run SDK/emulator tests or verify cloud behavior.
