...
Status: product direction agreed; detailed engineering specification independently reviewed. Ready for a separately requested implementation stage. Implementation has not started.
...
- Send state replaces review actions with progress and Stop. Results show `Confirmed sent`, `Failed`, `Outcome unknown`, `Not attempted`, with row number and MessageId. No automatic retry or misleading rollback label. View destination opens queue Active browsing or the existing combined topic view; it does not promise immediate visibility or delivery to every subscription.
...
- Properties: optional nullable string fields `contentType`, `subject`, `correlationId`, `sessionId`; applicationProperties is an object keyed by exact nonempty names. Omit unset fields in SDK mapping; preserve intentional string whitespace rather than using the legacy mapper's trimming. Empty sessionId is treated as unset and is invalid for a session-required destination.
- Application property descriptor: `type` is `string`, `boolean`, `int64` or `double`; `source` is a text expression. String supports interpolation; other types accept either a literal or exactly one token, then parse invariantly. Double must be finite. Null, arrays and objects are not supported application property values in v1. Names are literals, never templates.
...
- No destination, profile, original MessageId, scheduled time, TTL, credentials, absolute path, timestamps or broker observation metadata in the template schema. GUID id identifies the template; each new send gets a separate transport MessageId.
...
7. Generate a separate GUID MessageId per prepared row, displayed with preview/results. Never copy the observed source MessageId. Generated values and IDs remain stable when selecting rows, moving between tabs or revisiting Review. Only explicit rebuilding after an input/template change creates a new prepared plan; display that values will regenerate.
...
Application-property types have separate broker representations: int64 uses checked signed 64-bit conversion and rejects fractional, exponent and out-of-range input; double uses invariant IEEE-754 Double conversion, rejects nonfinite/overflow results, and previews its round-trip (`R`) representation. Finite rounding inherent in explicit double is visible in the frozen preview. A user needing exact large digits in an application property chooses string. Tests must distinguish these property rules from lossless JSON body numbers, including `120.00`, exponent spelling, high precision, CLR-overflow-sized body numbers, and property overflow.
...
Copy only contentType, subject, correlationId, sessionId and supported application properties. Normalize signed/unsigned integer CLR values into Int64 only when lossless, float into finite Double, plus existing string/bool/Int64/Double. Unsupported values (including oversized UInt64, binary, dates and custom types) appear in an exclusion list requiring explicit acknowledgement before saving; never silently stringify. The author can manually replace excluded values with supported types. Do not copy MessageId, lock token, sequence, delivery count, timestamps, DLQ reason/description or other system observation properties.
...
- `PreparedRun` owns run ID, immutable body UTF-8 bytes, normalized typed properties, per-row MessageId, row index and generation context. `ReviewedRun` additionally binds selected profile ID, namespace, connection generation, canonical EntityAddress and preflight result. Its approval is consumed once.
...
- Send one message at a time in CSV order, one SDK call per row. No transactions, concurrency, auto-retry, duplicate-detection guarantee or exactly-once claim. Stop at the first failure/uncertain result. Remaining rows become Not attempted. Completed means every send was acknowledged, not that every consumer processed it.
- A successful SDK return is Confirmed sent, even if later disposal, UI refresh, activity logging or result export fails. Local validation/preflight failures are definite unsent. Once a send call starts, cancellation, timeout or unclassified transport failure is conservatively Outcome unknown; do not turn it into Failed/Not attempted. A broker rejection may be classified Failed only when documented SDK evidence establishes rejection; otherwise use unknown.
...
- No durable outbox/resume in v1. A process crash can leave outcome unknown; show session-only result limitations. Provide optional user-selected JSON result export containing run/destination identity, row indices, MessageIds, timestamps and outcome codes, but no bodies, input values, credentials or raw exception dumps. Export failure does not change send outcomes. Never auto-save results into shared roots.
...
**Acceptance:** R1–R4 pass; literal JSON/text files survive reopen; healthy siblings remain usable; no app-generated credentials/absolute paths enter a template. **Stop:** current-shell or preference ownership conflicts, stronger filesystem guarantees requested, or schema divergence. **Prompt:** Implement Stage 1 only, derive red tests from R1–R4, integrate the real startup path, run the required gates, record proof and stop.
...
**Acceptance:** R9–R12 pass; provided example generates three distinct-ID messages with numeric amounts; invalid third row prevents all sending; stopped/unknown sends are never retried. **Stop:** grouped rows, scripts, parallel sending, durable resume or larger limits are requested; these require a new scope decision. **Prompt:** Implement Stage 3 only with R9–R12 red proof, reuse Stage 2 semantics, prove actual row outcomes and the whole UI flow, record results and stop.
...
- [ ] S4.1 Independently trace every agreed decision and R1–R14 to code and executed tests; add missing proof before completion.
...
No app-managed Git, remote URL browsing, collection marketplace, cloud sharing, script evaluation, loops/grouped CSV, scheduling/TTL editing, binary templates, arbitrary SDK application-property types, load testing, parallel publishing, durable job queue, automatic retries/resume, template deletion UI or unrelated legacy cleanup in v1. Existing replay/delete behavior remains separate.

Feature Definition of Done:

- [ ] Every agreed product decision is exercised through the production startup path.
- [ ] All four stage acceptance blocks and R1–R14 proof obligations are reconciled.
