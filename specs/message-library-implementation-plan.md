...
Status: product direction agreed; original specification and six-capability extension independently reviewed. Revised Message Workbench workspace mockup approved as a prototype plan on 2026-09-21. Feature implementation has not started; the existing throwaway in-app prototype is not production behavior. A separately requested implementation stage is still required.
...
6. Use a **dedicated Message Workbench workspace in the existing window**. The Workspaces switcher shows Investigation and Message Workbench at the left of the top toolbar; connection state then profile selector, Watch all and Settings sit at the right. Preserve drafts/selections when switching workspaces. Library authoring works offline.
...
10. Keep the namespace sidebar beside the original library/editor/prepare panes. Topic selection filters both trees; template selection applies its associations in reverse. Show topic filtering through the existing Namespaces search field, with an in-field clear action and no separate filter row. Hide unrelated branches; clearing search restores both trees without discarding drafts. Infer a single valid destination and show read-only Send to; use a picker only when a choice is needed. The topic context amendment defines R18.
11. Implement the **five active approved boards pixel-perfect**, under the [visual acceptance contract](message-library/design/README.md): revised Message Workbench Board A plus unchanged B–E. The earlier Board A, discarded compact images and layouts are not design authority.
...
Read the [approved five-board visual contract](message-library/design/README.md) and [topic context amendment](message-library/topic-context.md) before any UI stage. Revised Board A and original B–E are active; the earlier Board A, prior seven-board set, compact mockups and alternative layouts are superseded. The [new prototype preview](../tools/ServiceBusEmulatorExplorer.InvestigationPrototype/preview.png) and [UI language](ui-language.md) retain authority for existing chrome/tokens.

![Approved Message Workbench workspace](message-library/design/approved/01-message-workbench.png)

![Approved capture and properties](message-library/design/approved/03-capture-properties.png)

**Pixel-perfect fidelity is mandatory**, not an illustrative aspiration. Match the approved composition and the actual app's flat WPF color tokens; the generated Board A's pale-blue stain/wash is explicitly **not** a color target. Keep intentional blue actions/selection/accent, but do not implement blue haze, bloom, gradients or tinted shadows on neutral surfaces. Verify real WPF captures with geometry overlays and separate solid-color sampling against the Investigation workspace; repair discrepancies before completion. Only the visual contract's explicit generated-text, runtime-data and production-resource corrections are exceptions. No rendered implementation PASS is claimed by these images.

- Keep **Workspaces | Investigation | Message Workbench** at the left of the top toolbar and **connection state | profile selector | Watch all | Settings** at the right. Switching tabs does not disconnect, reset browsing, stop Watch or discard either workspace's drafts.
- Persistent far-left namespace pane retains its existing Search and Entities/Messages/Scheduled/DLQ counts/refresh. Topic context is represented in that Search field with an in-field clear action; there is no new full-width filter row. Beside it, the separate library pane has Add folder, Refresh libraries, search and roots → directories → templates. Library search is local name/description matching. Shared topic context prunes both projections under R18, retaining matching ancestors/subscriptions; no broker counts appear in the library tree.
...
Latest mockup approval (2026-09-21): the user approved the revised [Message Workbench workspace](message-library/design/approved/01-message-workbench.png) as the prototype/mockup plan. It retains four panes and the existing app palette, puts Workspaces/Investigation/Message Workbench on the left of the top toolbar, puts connection state/profile/Watch/Settings on the right, restores Scheduled counts, and uses the Namespaces Search instead of a separate topic-filter row. Boards B–E remain state references. The separately requested in-app dummy-data prototype has since been updated; see [prototype verification scope](message-library/design/README.md#dummy-data-prototype). Production implementation and its stage manifests remain Pending.

Prior approval update (2026-09-20): the user selected exactly five boards and required pixel-perfect implementation. The then-active visual contract and repository-local `design/approved/` images superseded earlier visual selections; eight obsolete repository mockups were deleted at the user's explicit request. R18 specified associations, bidirectional filtering, inferred destinations and preserved namespace refresh. Independent Luna review returned NO_FINDINGS after clarifying capture's MessageId as read-only. All 38 local links in those changed documents and five PNGs validated at that time. Production/render/accessibility proof remained Pending. Records below describe earlier revisions, not authority for discarded designs.
